...
public class TaildirMatcher {
    ...
    List<File> getMatchingFiles() {
        ...
        if (!cachePatternMatching ||
            ...
            //可以看到getMatchingFilesNoCache()就是获取匹配的文件的方法，也就是需要修改的方法(递归子目录)
//      lastMatchedFiles = sortByLastModifiedTime(getMatchingFilesNoCache());
    ...
    private List<File> getMatchingFilesNoCache() {
        ...
        //try()里面创建的文件流都实现了AutoCloseable接口，在语句结束后自动关闭
    ...
    private List<File> getMatchingFilesNoCache(boolean recursion) {
        logger.info("TaildirMatcher.recursion = " + recursion);
        if (!recursion) {
        ...
        logger.info("dirs.size = " + dirs.size());
        while (dirs.size() > 0) {
            File dir = dirs.poll();
        ...
        for (File file :
                result) {
            logger.info("result.file.path = " + file.getAbsolutePath());
        }
        return result;
    }

 ...
 * 这种模式涉及到一个作为桥接的抽象类，使得实体类的功能独立于接口实现类。这两种类型的类可被结构化改变而互不影响。
 * 在本例中：Shape就是桥接的抽象类，它有Circle作为其实体类，独立于DrawAPI接口的GreenCircle和RedCircle两个实现类
 * 在Shape中定义了一个带有DrawAPI参数的构造函数，用于桥接的时候传入具体的DrawAPI的实现类。
 * 桥接的作用是：可以使用相同的抽象类方法draw()但是不同的桥接实现类，来画出不同颜色的圆。
 *
